...
/**
 * Deep merge two objects (prototype-pollution safe with depth limit)
 ...
 */
function deepMerge(target, source, depth = 0, maxDepth = 50) {
  // Prevent stack overflow from deeply nested objects
  if (depth >= maxDepth) {
    console.warn(`Deep merge exceeded maximum depth (${maxDepth}), stopping recursion`);
    return target;
  }
  ...
  for (const key of Object.keys(source)) {
    ...
    if (sourceValue && typeof sourceValue === 'object' && !Array.isArray(sourceValue)) {
      ...
      if (targetValue && typeof targetValue === 'object' && !Array.isArray(targetValue)) {
        deepMerge(targetValue, sourceValue, depth + 1, maxDepth);
      } else {
...
function loadConfig(options = {}) {
  ...
  // Deep freeze the config to prevent mutations
  deepFreeze(config);

  // Cache the frozen result
  _configCache = config;
  _cacheTime = Date.now();
...
function validateConfig(config) {
  ...
  if (config.performance !== undefined) {
    if (typeof config.performance !== 'object' || config.performance === null || Array.isArray(config.performance)) {
  ...
  if (config.state !== undefined) {
    if (typeof config.state !== 'object' || config.state === null || Array.isArray(config.state)) {
  ...
  if (config.logging !== undefined) {
    if (typeof config.logging !== 'object' || config.logging === null || Array.isArray(config.logging)) {
