...
function runMultiPassAnalyzers(repoPath, targetFiles) {
  ...
  // Stub function analysis (per-file)
  const stubPattern = multiPassPatterns.placeholder_stub_returns_js;
  if (stubPattern) {
    for (const file of targetFiles) {
      // Skip test files and non-JS/TS files
      if (analyzers.isTestFile(file)) continue;
      if (!file.match(/\.(js|jsx|ts|tsx|mjs|cjs)$/i)) continue;
      ...
      for (const v of stubViolations) {
        findings.push({
          file,
          line: v.line,
          patternName: 'placeholder_stub_returns_js',
