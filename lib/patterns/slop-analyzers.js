...
function analyzeStubFunctions(content, options = {}) {
  ...
  // Only analyze JS/TS for now (can extend to other languages)
  if (lang !== 'js') return violations;

  const lines = content.split('\n');

  // Patterns to find function declarations
  // Group 1: function name (if available)
  const functionPatterns = [
    ...
    // Method definitions: name() { or async name() { - exclude keywords
    /^\s*(?:async\s+)?(?!if\b|for\b|while\b|switch\b|catch\b|with\b)(\w+)\s*\([^)]*\)\s*\{/gm,
  ];

  // Stub return values
  const stubReturnPattern = /^\s*return\s+(0|null|undefined|true|false|\[\]|\{\}|""|''|``)\s*;?\s*$/;
  ...
  for (const pattern of functionPatterns) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      // Check if body contains only a stub return
      if (significantLines.length === 1) {
        const onlyLine = significantLines[0].trim();
        const stubMatch = stubReturnPattern.exec(onlyLine);

        if (stubMatch) {
          const returnValue = stubMatch[1];

          // Check for TODO/FIXME in function body or nearby
          ...
          violations.push({
            ...
            content: `${funcName}() { return ${returnValue}; }`
          });
        }
      }
    }
